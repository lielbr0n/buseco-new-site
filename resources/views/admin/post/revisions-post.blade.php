        ...
        <div class="w-full pl-1 pr-1 sm:pr-2 sm:pl-0">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-3 text-gray-900">
                    <h1>Title: <b>{{ $postTitle }}</b></h1>
                    @forelse($postLogsInfo as $postLogs)
                        <div class="font-medium">{{ date_format(new DateTime($postLogs->date_created),"F d, Y h:i:s a"); }}</div>
                        <div class="flex">
                            <span>Updated by: <b>{{$postLogs->username}}</b>.</span>
                            <span class="text-[#ff0000] ml-1">
                                modified fields:
                                <b>
                                    @php 
                                        $modified_fields = unserialize($postLogs->modifiedFields);  
                                        $modified_field_count = count($modified_fields);
                                        for($i = 0; $i < $modified_field_count; $i++){
                                        echo $modified_field_count > 1 ? array_keys($modified_fields)[$i] .', ' : array_keys($modified_fields)[$i];
                                        }
                                    @endphp
                                </b>
                            </span>
                           
                            <div class="text-[#52b3ce] ml-5 underline font-bold">
                                <a href="{{route('post.revisions.modified.fields', ['postLogId' => $postLogs->id])}}">View</a>
                            </div>
                        </div>
                       
                    @empty
                        <div>No revision(s) found</div>
                    @endforelse
