                                                ...
                                                <tr class="border-b border-neutral-200 dark:border-white/10">
                                                    ...
                                                    <td class="whitespace-nowrap px-2 py-1 text-center font-semibold">
                                                        <span @class([
                                                            ...
                                                            "bg-warning" => $post->post_status === 'draft',
                                                            "bg-success" => $post->post_status === 'publish'
                                                            ])
                                                        >{{ strtoupper($post->post_status) }}
                                                        </span>
                                                    </td>
                                                    <td class="whitespace-nowrap px-2 py-1 text-center font-medium">{{ $post->created_at }}</td>
                                                    <td class="whitespace-nowrap px-2 py-1 text-center font-medium">{{ $post->updated_at }}</td>
                                                    <td class="whitespace-nowrap px-2 py-1 text-center font-medium flex justify-center">
                                                        <a href="{{route('post.edit', ['postId' => $post->post_id])}}" class="border-2 border-info p-3 bg-info text-white hover:text-info hover:bg-white ">
                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
                                                                <path d="M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32L19.513 8.2Z" />
                                                            </svg>
                                                        </a>
                                                        <a href="{{route('post.delete', ['postId' => $post->post_id])}}" onclick="return confirm('Are you sure you want to delete this post?')" class="border-2 border-danger ml-2 p-3 bg-danger text-white hover:text-danger hover:bg-white">
                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
